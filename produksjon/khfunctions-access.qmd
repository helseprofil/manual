---
title: "KHELSA.mdb"
description: Informasjon om access-databasen knyttet til khfunctions
editor: visual
---
...
**OBS!** Denne filen er en konvertering fra *.docx*-filen på F-disk til *.md*. Layout (overskriftsstiler etc) er litt rotete, rydding er bare påbegynt. Bruk gjerne *søk*-funksjonen ovenfor for å finne det du leter etter. Du kan også se på [pdf-fil](https://helseprofil.github.io/extra/Bruksanvisning.pdf)
...
[Kubeproduksjon (dvs. all output)](#kubeproduksjon-dvs.-all-output)

[Eksempel på kompleks indikator: INNTULIKHET]

[Sammenhengen mellom fil-elementene (til bruk ved f.eks. kval.kontroll)](#sammenhengen-mellom-fil-elementene-til-bruk-ved-f.eks.-kval.kontroll)
...
+-----------------------------------+--------------------------+-----------------------+
| **Alle kommuner,**\               | **Bare bydeler for de**\ | **Noen har seks**\    |
| **PLUSS bydeler:**                | **aktuellekommunene,**\  | **sifre hele veien:** |
|                                   | **vanlig koding:**       |                       |
+:==================================+:=========================+:======================+
| 0104                              | 0104                     | 010400                |
+-----------------------------------+--------------------------+-----------------------+
...
1.  Øverst er en oppsummering: Etter litt basisinfo om inndatafilen kommer raden som starter med «INNLES_OK». **Alle feltene i denne raden skal ha verdien 1** (eller blank, hvis f.eks. den aktuelle variabelen ikke fins).
...
+:====+:=============+:==========================================+
| A   | Alder        | 888_888 Ugyldig, 999_999 Ukjent           |
+-----+--------------+-------------------------------------------+
| Gn  | Geonivå      | L land, H Hreg, F fylke, K kommune,\      |
|     |              | B bydel, S sone, G grunnkrets, U ugyldig. |
+-----+--------------+-------------------------------------------+
| K   | Kjønn        | 0 kjønn samlet, 1 menn, 2 kvinner,\       |
|     |              | 8 ugyldig, 9 ukjent.                      |
+-----+--------------+-------------------------------------------+
...
> \> k\<-LagKubeDatertCsv("BARN_SOSHJELP")
>
> BATCH: 2020-12-18-14-33
>
> Hentet FIL BARN_SOSHJELP fra BUFFER (22903 x 16)
...
>
> \[Lest inn\]{.underline} FIL BARN_SOSHJELP (22920 x 17), batch=\[2020-12-18-14-16\]{.underline}
...
TNP_NAVN Vi lager et utfilnavn. Det er ofte lik tellerfil-navnet.
...
NYEKOL_RAD Nye kolonner fra rader: Lag ny kolonne med \<navn\> ut fra verdiene i \<denne variabelen\> når \<denne tab-kolonnen\> har \<disse verdiene\>.

Syntaks:

Nevner=ANTALL{TAB1=="aperinat_fodt_500_who"\| TAB1=="aperinat_fodt_1000_who"}\
...
Kan oppgi \<navn\> fra NYEKOL_RAD-kommandoen ovenfor!
...
>
> Ved indirekte standardisering (se pkt. a) betyr dette at det trengs «predteller»^[1](https://helseprofil.github.io/manual/khfunctions-access.html#fn:1)^ basert på de aldersspesifikke ratene på landsnivå samme år. Siden de ordinære Friskvikfilene for fylkesprofiler kjøres ut med fast referanse-år (2012), vil alle predtellere i disse filene være basert på de aldersspesifikke ratene på landsnivå i 2012, hvilket ikke er det som trengs. Situasjonen krever at det kjøres ut ekstra filer med løpende standardisering. Altså: Riktig MEIS^[2](https://helseprofil.github.io/manual/khfunctions-access.html#fn:2)^ må komme fra de ordinære filene standardisert mot fast standard-år (for at MEIS i profilene skal stemme med MEIS i statistikkbanken), mens rett predteller (og følgelig Forholdstall og figurverdier) må komme fra ekstrafilene standardisert mot løpende standard-år.
>
...
Kommando LagFilgruppe eller LagKube med parameter "dumps", se i SePaaFil.r. Se liste over punktene i [\\\\fhi.no\\Felles\\Forskningsprosjekter\\PDB 2455 - Helseprofiler og til\_\\PRODUKSJON\\DOK\\DUMPPUNKTER.docx](file://fhi.no/Felles/Forskningsprosjekter/PDB%202455%20-%20Helseprofiler%20og%20til_/PRODUKSJON/DOK/DUMPPUNKTER.docx)
...
*write.table(FG, file='F:\\\\\*\*Forskningsprosjekter\\\\* *PDB 2455 - Helseprofiler og til\_\\\\PRODUKSJON\\\\RUNTIMEDUMP\\\\EIERSTATUS_FG.csv', sep=';', na="", row.names = FALSE)*
...
+:=========================================+:=========================================+:=======================================+
| Ferdig kubedatafil til statistikkbanken\ | Defineres i tabell KUBER – KUBE_NAVN.    | Bygger på en TNP-fil, angitt\          |
| ("kube", output fra R-systemet)          |                                          | i KUBER – TNP.                         |
+------------------------------------------+------------------------------------------+----------------------------------------+
| TNP-fil                                  | Defineres i tabell TNP_PROD – TNP_NAVN.  | Bygger på en tellerfil, angitt i\      |
|                                          |                                          | TNP_PROD – TELLERFIL,\                 |
|                                          |                                          | og evt. en nevnerfil angitt i\         |
|                                          |                                          | – NEVNERFIL.                           |
+------------------------------------------+------------------------------------------+----------------------------------------+
| Tellerfilen =en filgruppe                | Defineres normalt i tabell FILGRUPPER\   | Bygger på én eller flere inndatafiler\ |
|                                          | – FILGRUPPE, som regel i skjemaet\       | (ORGfiler), som angis i – FILNAVN.\    |
|                                          | STYRING\\StyrFILGRUPPER øverste tabell.\ | Disse har hver sin IBrukFra og\        |
|                                          | (Kan evt. defineres i FILFILTRE, dvs.\   | IBrukTil-dato.                         |
|                                          | lages runtime. Det er mer aktuelt\       |                                        |
|                                          | for nevnerfilen.)                        |                                        |
+------------------------------------------+------------------------------------------+----------------------------------------+
...
Tabell KUBESTATUS (KH2020_KUBESTATUS etc) viser prosessen med kvalitetskontroll av ferdig kube.
...
|  -1  | Norge                           |
| 100  | Innvandrere                     |
|  1B  | Europa unntatt Tyrkia_innvand   |
|  1C  | Europa unntatt Tyrkia_norskf    |
|  2B  | Afrika_innvand                  |
|  2C  | Afrika_norskf                   |
|  3B  | Asia med Tyrkia_innvand         |
|  3C  | Asia med Tyrkia_norskf          |
|  4B  | Nord- Amerika_innvand           |
|  4C  | Nord- Amerika_norskf            |
|  5B  | Sør- og Mellom- Amerika_innvand |
|  5C  | Sør- og Mellom- Amerika_norskf  |
|  6B  | Oseania_innvand                 |
|  6C  | Oseania_norskf                  |

## **Sivilstand, koder i KH_KODER:**
